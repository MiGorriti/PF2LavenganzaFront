...
import { FILTER_CATEGORY, FILTER_LOCATION, GET_CATEGORYS, GET_LOCATIONS, GET_PROPERTY, POST_PROPERTY, GET_DETAIL, CREATE_USER, LOGIN_USER, SET_USER_NAME, } from './type-actions'
...
export const createUser = (postForm) => {
  return async function (dispatch) {
    try {
      ...
      const fullName = postForm.fullName;
      dispatch(setUserName(fullName));
      console.log( "nombre createUser",fullName);
  ...
  return async function (dispatch) {
    try {
      ...
      if (response.status === 200) {
        ...
        console.log(`Nombre del usuario obtenido: ${fullName}`)
        ...
        dispatch(setUserName(fullName)); // Dispatch la acción para actualizar el nombre de usuario
      }
    } catch (error) {
      dispatch({
        type: 'LOGIN_USER',
